...
export const ProductDetailsBox: React.FC<ProductProps> = ({
	...
	// index of selected product color
					...
					<div className="flex px-5 items-center md:items-start flex-col w-full h-full">
						<div className="flex flex-col items-start md:gap-y-8 gap-y-4 justify-center h-2/3 w-full lg:w-2/3">
							...
							<div className="flex w-3/4 mx-auto justify-between md:gap-y-8 md:gap-x-0 gap-x-20 items-center flex-row">
								...
								<div className="flex flex-row items-center justify-end gap-x-1.5 md:gap-x-3">
									<span className="text-black/75 font-black">رنگ:</span>
									{pricePerColor?.map(({ productColor }, index: number) => (
										<span
											onClick={() => setSelectedColor(index)}
											key={index}
											className={`border-Dark/80 border block md:h-6 md:w-6 h-5 w-5 duration-500 rounded-full cursor-pointer ${index === selectedColor ? 'rounded-sm border-DarkYellow border-2' : null}`}
						...
						<div className="flex justify-center gap-y-8 items-center w-full flex-col md:flex-row">
							<div className="flex md:leading-6 text-Dark/60 text-justify mt-6 xl:mt-8 md:w-1/2 tracking-tight">
								<span
									dangerouslySetInnerHTML={{ __html: description! }}
									className="line-clamp-6 md:text-lg text-xs"
								></span>
							</div>
							<div className="md:flex flex-col w-1/2 gap-y-1 items-end px-10 justify-center hidden">
								<div className="flex flex-row gap-x-4 items-center">
									{pricePerColor ? (
										pricePerColor[selectedColor].finalPrice !==
										pricePerColor[selectedColor].price ? (
											<span className="flex justify-center items-center rounded-lg bg-gradient-to-l from-red-500 to-red-600 pt-[3px] text-center text-[10px] font-bold text-white/70 h-6 w-10 text-sm">
												{Math.trunc(
													100 -
														(pricePerColor[selectedColor].finalPrice! * 100) /
															pricePerColor[selectedColor].price!
								...
								{pricePerColor ? (
									pricePerColor[selectedColor].finalPrice !== pricePerColor[selectedColor].price ? (
										<div className="flex flex-row items-center gap-x-2">
											<span className="font-Lalezar mt-1 text-2xl line-through text-Dark/70">
												{pricePerColor[selectedColor].price!.toLocaleString('fa-IR')}
											</span>
									...
									>
										<div className="flex gap-x-3 items-center justify-between">
											<span className="font-Lalezar mt-1 text-2xl">
												{pricePerColor
													? pricePerColor[selectedColor].finalPrice?.toLocaleString('fa-IR')
													: null}
											</span>
